...
class Main:
    def __init__(self, sig_bit, plane, key, paths, shared_methods):
        ...
        unpacked_bits = np.unpackbits(np.fromfile(self.data_image_path, dtype="uint8"))
    ...
    def watermark(self):
        secret_watermark = os.path.splitext(self.save_path)[1][1:] + "/dylan/" + date.today().strftime(
            "%Y-%m-%d")
        secret_watermark = ''.join(format(ord(i), 'b').zfill(8) for i in secret_watermark)
        random_index = random.randint(0, len(self.data_image_bits))
        return self.data_image_bits[:random_index] + secret_watermark + self.data_image_bits[random_index:]
